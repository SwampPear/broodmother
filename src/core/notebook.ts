...
// The cell's original JSON with only what changed written over it, so metadata and keys this
// codec has no opinion about ride through untouched. A cell whose type switched keeps only
// its metadata: the rest of its shape belongs to the type it no longer is.
...
// Merges the model back into the JSON it was parsed from: cells are matched by id, new cells
// are written fresh, and everything unmatched by an edit — notebook metadata, cell metadata,
// unknown keys — is carried verbatim. A notebook nobody changed comes back as the very bytes
// that went in.
