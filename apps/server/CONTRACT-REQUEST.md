...
`RoomId` is documented as `${repoId}/${vaultPath}` but `MotherConfig` exposes nothing to build
...
stable and readable instead, add a `repoId: string` to `MotherConfig` and the relay will use it
verbatim.
...
## 5. Config file location vs `MotherConfig.vaultPath`

The config file is `<bootstrap root>/.mother/config.json`, where the bootstrap root is
`MOTHER_VAULT` or the process cwd; `config.vaultPath` then selects the vault that documents are
read from and defaults to that same root. With no `MOTHER_VAULT` set the two coincide and the
