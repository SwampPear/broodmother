...
import { scheduleLines, type Crontab, type ScheduledDream } from './crontab'
 ...
 * running when the clock strikes and cron will be. The lair is a long-lived process, so
 * it keeps time itself and fires the run in-process.
 */
export interface Scheduler {
  sync(found: ScheduledDream[]): Promise<void>
}
