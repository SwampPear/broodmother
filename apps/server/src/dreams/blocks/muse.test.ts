...
import type { MuseNode } from '@broodmother/shared'
...
it('runs headlessly with the flow protocol spelled out', async () => {
  ...
  expect(args[2]).not.toContain('$DREAM')
...
it('rides the brief ahead of the prompt', async () => {
  const { dir, ctx } = await ctxAt({ brief: 'the vault map' })
  await museBlock(nodeAt(), ctx)
  const args = await argsOf(dir)
  expect(args[1]).toBe('--yolo')
  expect(args[2].startsWith('the vault map\n\nsum up')).toBe(true)
})
