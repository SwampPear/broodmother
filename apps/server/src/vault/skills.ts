...
/** A reusable workflow the vault carries: a folder under `.skills/` in the Claude skills
...
export async function scanSkills(checkout: string): Promise<Skill[]> {
  ...
  for (const dirent of dirents) {
    ...
    skills.push({ name: dirent.name, description: descriptionOf(skill) })
  }
  return skills.sort((a, b) => a.name.localeCompare(b.name))
}

function descriptionOf(skill: string): string {
  const fence = skill.match(FENCE)
  const line = fence?.[1]
    .split('\n')
