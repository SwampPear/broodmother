...
- Per-agent files are tool-specific only. Anything true for every agent goes in
  `.claude/CLAUDE.md`, in `LESSONS.md`, or in `README.md`/`DESIGN.md`, and the agent file
  points at it rather than restating it.
- A lesson that only holds for one package belongs next to that package, not in
  `LESSONS.md`.
