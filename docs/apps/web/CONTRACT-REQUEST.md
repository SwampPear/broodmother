...
it what `repoId` is — `DocsConfig` has no such field and `GET /api/config` returns only the
...
- add `repoId: string` to `DocsConfig` (or to the `GET /api/config` response), or
