...
export function defaultConfig(vaultPath: string): DocsConfig {
  const user = os.userInfo().username || 'docs'
  return {
    vaultPath,
    remoteUrl: null,
    branch: 'main',
 ...
 */
export function repair(raw: unknown, defaults: DocsConfig): LoadedConfig {
  const source =
  ...
  return { config: config as unknown as DocsConfig, reset }
}

export class ConfigStore {
  private current: DocsConfig
  ...
  async save(config: DocsConfig): Promise<DocsConfig> {
